...
export function WorkspaceOverlay({ kind, channels, messages, savedMessages, onSelectChannel, onOpenThread, onClose, memberCount, connection, threadItems: loadedThreadItems }: Props) {
  ...
  const localThreadItems = allMessages.filter(({ message }) => (message.threadCount ?? 0) > 0)
  const threadItems = loadedThreadItems ?? localThreadItems
  ...
  return <div className="workspace-overlay-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose() }}>
    <section className="workspace-overlay" role="dialog" aria-modal="true" aria-label={kind === 'search' ? t('overlay.searchDialog') : t(`overlay.titles.${kind}`)}>
      ...
      {kind === 'workspace' && <div className="workspace-info-card"><div className="workspace-info-icon">LL</div><h3>{t('brand.workspaceName')}</h3><div className="workspace-info-grid"><div><strong>{channels.length}</strong><small>{t('overlay.conversationCount')}</small></div><div><strong>{memberCount ?? '—'}</strong><small>{t('overlay.memberCount')}</small></div><div><strong className={`workspace-connection workspace-connection-${connection}`}><span aria-hidden="true">●</span> {t(`chat.${connection}`)}</strong><small>{t('overlay.connectionState')}</small></div></div></div>}

      {kind === 'help' && <div className="help-list"><div className="help-row"><span>{t('overlay.helpSearch')}</span><kbd>⌘ / Ctrl + K</kbd></div><div className="help-row"><span>{t('overlay.helpClose')}</span><kbd>Esc</kbd></div><div className="help-row"><span>{t('overlay.helpSend')}</span><kbd>Enter</kbd></div><div className="help-row"><span>{t('overlay.helpNewline')}</span><kbd>Shift + Enter</kbd></div></div>}
