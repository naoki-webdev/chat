    ...
    <section className="workspace-overlay channel-create-dialog" role="dialog" aria-modal="true" aria-label={t('channel.createTitle')}>
      ...
      <div className="channel-create-body">
        <form onSubmit={submit}>
          ...
          <div className="channel-create-field"><span>{t('channel.membersLabel')}</span><div className="channel-member-picker">{members.filter((member) => member.id !== currentUserId).map((member) => {
          ...
          })}{members.filter((member) => member.id !== currentUserId).length === 0 && <p className="channel-member-empty">{t('channel.membersEmpty')}</p>}</div><small className="channel-create-hint">{t('channel.membersHint')}</small></div>
