...
export function ProfileMenu({ user, presence, onChangePresence, onUpdateProfile, onLogout, onClose }: Props) {
  ...
  return <section className="profile-menu" role="dialog" aria-label={t('profile.title')}>
    ...
    <button type="button" className="profile-logout" onClick={() => { onClose(); onLogout() }}>{t('profile.logout')}</button>
  </section>
}
