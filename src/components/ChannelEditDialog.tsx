...
export function ChannelEditDialog({ channel, members, channelMembers, currentUserId, currentUserRole, onSave, onClose }: Props) {
  ...
  const memberIDsForSave = useMemo(() => Array.from(new Set([currentUserId, ...channelMembers.filter((member) => !member.is_bot).map((member) => member.id)])), [channelMembers, currentUserId])
    ...
    <section className="workspace-overlay channel-edit-dialog" role="dialog" aria-modal="true" aria-label={t('channel.editTitle')}>
      ...
      <div className="channel-create-body">
        <form onSubmit={submit}>
          ...
          <div className="channel-create-field"><div className="channel-field-label"><span>{t('channel.membersLabel')}</span><button className="inline-help" type="button" title={t('channel.membersHelp')} aria-label={t('channel.membersHelp')}>?</button></div><div className="channel-member-picker">{members.map((member) => {
          ...
          })}{members.length === 0 && <p className="channel-member-empty">{t('channel.membersEmpty')}</p>}</div></div>
