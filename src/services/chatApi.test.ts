...
describe('chatApi', () => {
  ...
  it('forwards an AbortSignal to message reloads', async () => {
    fetchMock.mockResolvedValueOnce(jsonResponse({ messages: [], has_more: false, cursor: 43 }))
    const controller = new AbortController()

    await chatApi.listMessages('general', undefined, 50, controller.signal)

    expect(fetchMock).toHaveBeenCalledWith(
      'http://127.0.0.1:8080/api/channels/general/messages?limit=50',
      expect.objectContaining({ credentials: 'include', signal: controller.signal }),
    )
  })

  it('forwards an AbortSignal to channel member reloads', async () => {
    fetchMock.mockResolvedValueOnce(jsonResponse({ members: [] }))
    const controller = new AbortController()

    await chatApi.listChannelMembers('general', controller.signal)

    expect(fetchMock).toHaveBeenCalledWith(
      'http://127.0.0.1:8080/api/channels/general/members',
      expect.objectContaining({ credentials: 'include', signal: controller.signal }),
    )
  })
  ...
  it('converts API errors into ChatApiError', async () => {
    fetchMock.mockResolvedValueOnce(jsonResponse({ error: 'authentication required', code: 'UNAUTHORIZED', request_id: 'request-123' }, 401))

    await expect(chatApi.me()).rejects.toEqual(expect.objectContaining<ChatApiError>({ name: 'ChatApiError', status: 401, message: 'authentication required', code: 'UNAUTHORIZED', requestId: 'request-123' }))
  })
  ...
  it('requests older thread replies with a cursor', async () => {
    fetchMock.mockResolvedValueOnce(jsonResponse({ messages: [], has_more: false, cursor: 5 }))

    await chatApi.listThreadMessages('m-1', '12', 20)

    expect(fetchMock).toHaveBeenCalledWith(
      'http://127.0.0.1:8080/api/messages/m-1/replies?limit=20&before=12',
      expect.objectContaining({ credentials: 'include' }),
    )
  })

  it('requests thread roots for the workspace index', async () => {
    fetchMock.mockResolvedValueOnce(jsonResponse({ messages: [], total: 0 }))

    await chatApi.listThreadRoots(100)

    expect(fetchMock).toHaveBeenCalledWith(
      'http://127.0.0.1:8080/api/threads?limit=100',
      expect.objectContaining({ credentials: 'include' }),
    )
  })
})
