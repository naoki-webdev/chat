...
GET    /api/events?after=:sequence  (要ログイン・差分同期)
GET    /api/threads                 (要ログイン・スレッド一覧)
...
最終回答は `messages` と `chat_events` に通常のメッセージとして保存します。開発・テスト環境ではProvider未設定またはAPIキー未設定時にモックプロバイダへフォールバックします。本番環境ではAI Providerと認証情報の設定が必要です。設定時はGoサーバーからOpenAI互換のストリーミングAPIを呼び出します。APIキーはフロントエンドへ返しません。
...
PostgreSQL構成では、`chat_events`への保存時に`LISTEN / NOTIFY`でイベントの発生を各Goインスタンスへ通知します。通知は配送の起点に限定し、正本はPostgreSQLとsequenceです。listenerの再接続や通知の取りこぼしは、sequence以降のイベントを読み直して補完します。

WebSocketの送信バッファが満杯になったslow clientは切断します。フロントエンドは再接続時に`/api/events?after=`から差分を取得して回復します。global sequenceは権限外イベントで飛ぶため、接続中のsequenceの飛びは欠落とみなしません。

メッセージ履歴は `?limit=50&before=:cursor` で古いページを取得できます。全チャンネルのリアルタイム差分は `GET /api/events?after=:sequence` で取得し、`POST /api/channels/:channel_id/read` で`channel_read_states.last_read_sequence`を更新します。`last_read_message_id`は参照用に保持します。
...
$env:DATABASE_URL = "postgres://orbit:orbit@127.0.0.1:5434/orbit_chat?sslmode=disable"
go run ./cmd/server
```
...
デモログインは`APP_ENV=development`または`APP_ENV=test`でデモseedを有効にした場合だけ利用できます。本番環境ではmigrationのみ実行され、既知パスワードのユーザーは自動作成されません。パスワードはbcryptでハッシュ化され、Cookieにはパスワードやユーザー情報を保存しません。
