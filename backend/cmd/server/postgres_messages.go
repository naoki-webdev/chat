...
func (r *postgresRepository) MarkChannelRead(ctx context.Context, userID, channelID string) (int64, error) {
	...
	err := r.upsertChannelReadState(ctx, userID, channelID, latest, latestMessageID)
	return latest, err
}

func (r *postgresRepository) upsertChannelReadState(ctx context.Context, userID, channelID string, sequence int64, messageID string) error {
	_, err := r.pool.Exec(ctx, `INSERT INTO channel_read_states (user_id,channel_id,last_read_sequence,last_read_message_id) VALUES ($1,$2,$3,$4) ON CONFLICT (user_id,channel_id) DO UPDATE SET last_read_sequence=GREATEST(channel_read_states.last_read_sequence, EXCLUDED.last_read_sequence), last_read_message_id=CASE WHEN EXCLUDED.last_read_sequence > channel_read_states.last_read_sequence THEN EXCLUDED.last_read_message_id ELSE channel_read_states.last_read_message_id END, updated_at=now()`, userID, channelID, sequence, nullableString(messageID))
	return err
}
...
func (r *postgresRepository) DeleteMessage(ctx context.Context, messageID, userID string) (string, EventRecord, error) {
	...
	var deletedParentMessageID string
	if err := transaction.QueryRow(ctx, `DELETE FROM messages WHERE id=$1 RETURNING COALESCE(parent_message_id,'')`, messageID).Scan(&deletedParentMessageID); err != nil {
		if errors.Is(err, pgx.ErrNoRows) {
			return "", EventRecord{}, ErrNotFound
		}
		return "", EventRecord{}, err
	}
	if deletedParentMessageID != "" {
		if _, err := transaction.Exec(ctx, `UPDATE messages SET thread_count=GREATEST(thread_count-1,0) WHERE id=$1`, deletedParentMessageID); err != nil {
			return "", EventRecord{}, err
		}
	}
	record, err := appendEventTx(ctx, transaction, realtimeEvent{Type: "message.deleted", ChannelID: channelID, MessageID: messageID, ParentMessageID: deletedParentMessageID})
	if err != nil {
		return "", EventRecord{}, err
	}
	if err := transaction.Commit(ctx); err != nil {
