...
SELECT c.id, c.name, c.channel_group, c.kind, c.description, COALESCE(c.dm_peer_user_id,''), COALESCE(c.presence,''), COALESCE(c.initials,''), COALESCE(c.color,''),
       COALESCE((SELECT count(*) FROM messages unread_message WHERE unread_message.channel_id=c.id AND unread_message.deleted_at IS NULL AND unread_message.author_id<>$1 AND unread_message.created_sequence > COALESCE((SELECT last_read_sequence FROM channel_read_states WHERE user_id=$1 AND channel_id=c.id),0)),0)
	...
	for rows.Next() {
		var channel Channel
		if err := rows.Scan(&channel.ID, &channel.Name, &channel.Group, &channel.Kind, &channel.Description, &channel.PeerUserID, &channel.Presence, &channel.Initials, &channel.Color, &channel.Unread); err != nil {
			return nil, 0, err
		}
