...
func (r *memoryRepository) SearchMessages(_ context.Context, channelID, query string, limit int) ([]Message, error) {
	r.mu.RLock()
	defer r.mu.RUnlock()
	...
	for _, message := range r.messages[channelID] {
		if message.ParentMessageID != "" || message.Deleted {
			continue
		}
		author := r.users[message.AuthorID].User
		if !strings.Contains(strings.ToLower(message.Body), query) &&
			!strings.Contains(strings.ToLower(author.Name), query) &&
			!strings.Contains(strings.ToLower(author.Handle), query) {
			continue
		}
		message.Sequence = r.messageSequences[message.ID]
		results = append(results, cloneMessage(message))
	}
	sort.SliceStable(results, func(left, right int) bool { return results[left].Sequence < results[right].Sequence })
...
func (r *memoryRepository) ListThreadRoots(_ context.Context, userID string, limit int) (ThreadRootPage, error) {
	r.mu.RLock()
	defer r.mu.RUnlock()
	limit = normalizeLimit(limit)
	...
	for _, channel := range r.channels {
		...
		for _, message := range r.messages[channel.ID] {
			if message.ParentMessageID == "" && message.ThreadCount > 0 {
				message.Sequence = r.messageSequences[message.ID]
