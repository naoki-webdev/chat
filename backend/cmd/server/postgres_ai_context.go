...
func (r *postgresRepository) ListAIContextMessages(ctx context.Context, channelID string, limit int) ([]Message, error) {
	...
	rows, err := r.pool.Query(ctx, messageSelect+` WHERE m.channel_id=$1 AND m.deleted_at IS NULL ORDER BY m.created_sequence DESC LIMIT $2`, channelID, limit)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (r *postgresRepository) ListUnreadMessageContext(ctx context.Context, userID, channelID string, limit int) ([]Message, int, error) {
	...
	var unreadCount int
	if err := transaction.QueryRow(ctx, `SELECT count(*) FROM messages m WHERE m.channel_id=$1 AND m.deleted_at IS NULL AND m.author_id<>$2 AND m.created_sequence > COALESCE((SELECT last_read_sequence FROM channel_read_states WHERE user_id=$2 AND channel_id=$1),0)`, channelID, userID).Scan(&unreadCount); err != nil {
		return nil, 0, err
	}
	query := messageSelect + ` WHERE m.channel_id=$1 AND m.deleted_at IS NULL AND m.author_id<>$2 AND m.created_sequence > COALESCE((SELECT last_read_sequence FROM channel_read_states WHERE user_id=$2 AND channel_id=$1),0) ORDER BY m.created_sequence DESC`
