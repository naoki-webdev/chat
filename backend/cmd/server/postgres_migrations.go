...
func (r *postgresRepository) seed(ctx context.Context) error {
	...
	for _, peer := range []struct{ channelID, userID string }{
		{channelID: "ayaka", userID: "u-ayaka"},
		{channelID: "ken", userID: "u-ken"},
		{channelID: "orbit-ai", userID: orbitAIUserID},
	} {
		if _, err := r.pool.Exec(ctx, `UPDATE channels SET dm_peer_user_id=$2 WHERE id=$1 AND kind='dm' AND dm_peer_user_id IS NULL`, peer.channelID, peer.userID); err != nil {
			return err
		}
	}
