...
func newProductionServer(ctx context.Context) (*server, error) {
	...
	listenerContext, listenerCancel := context.WithTimeout(ctx, realtimeRepositoryTimeout)
	defer listenerCancel()
	if err := repository.waitForEventListener(listenerContext); err != nil {
		repository.Close()
		return nil, fmt.Errorf("postgres event listener did not become ready: %w", err)
	}
...
func (s *server) handler() http.Handler {
	...
	mux.HandleFunc("/api/threads", s.handleThreadRoots)
...
func main() {
	...
	httpServer := newHTTPServer(port, application.handler())
	go func() {
