...
type summaryItem struct {
	...
	SourceParentMessageID string `json:"source_parent_message_id,omitempty"`
}
...
`, scopeLabel)
	...
	observabilityLogger.Info("ai_request",
		"request_id", requestIDFromContext(parent),
		"user_id", userID,
	...
	if parsed.Chatter != nil {
		result.ChatterCount = maxInt(0, minInt(*parsed.Chatter, len(history)))
	} else {
...
func attachSummarySources(result *channelSummary, history []ai.Message) {
	...
	parentByID := make(map[string]string, len(history))
	for _, message := range history {
		id, _ := summarySource(message.Body)
		if id != "" {
			allowed[id] = struct{}{}
			parentByID[id] = summaryParent(message.Body)
		}
	}
	sanitize := func(items []summaryItem) []summaryItem {
		for index := range items {
			if _, exists := allowed[items[index].SourceMessageID]; !exists {
				...
				items[index].SourceParentMessageID = ""
				continue
			}
			items[index].SourceParentMessageID = parentByID[items[index].SourceMessageID]
		}
		return items
	}
...
func summaryParent(value string) string {
	const marker = "thread_parent="
	start := strings.Index(value, marker)
	if start < 0 {
		return ""
	}
	parent := strings.Fields(value[start+len(marker):])
	if len(parent) == 0 {
		return ""
	}
	return strings.TrimSuffix(parent[0], "]")
}
