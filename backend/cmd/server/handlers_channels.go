...
func (s *server) handleChannelRoutes(writer http.ResponseWriter, request *http.Request) {
	...
	case http.MethodGet:
		...
		searchQuery := strings.TrimSpace(request.URL.Query().Get("q"))
		if searchQuery != "" {
			if len([]rune(searchQuery)) > 100 {
				writeError(writer, http.StatusBadRequest, "search query must be 100 characters or fewer")
				return
			}
			results, err := s.repository.SearchMessages(request.Context(), channelID, searchQuery, limit)
			if err != nil {
				writeRepositoryError(writer, err)
				return
			}
			writeJSON(writer, http.StatusOK, MessagePage{Messages: results, HasMore: false})
			return
		}
	...
	case http.MethodPost:
		...
		if shouldInvokeAI(channelID, payload.Body) {
			go s.startAIReply(requestIDFromContext(request.Context()), channelID, user.ID, message)
		}
