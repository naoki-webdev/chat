...
func TestCursorPaginationReadStateAndEventSync(t *testing.T) {
	...
	otherCookie := registerTestUser(t, handler, "cursor-other@example.com")
	...
	otherPayload, _ := json.Marshal(messageRequest{Body: "message from another user"})
	otherMessageRecorder := httptest.NewRecorder()
	handler.ServeHTTP(otherMessageRecorder, authorizedRequest(http.MethodPost, "/api/channels/general/messages", otherPayload, otherCookie))
	if otherMessageRecorder.Code != http.StatusCreated {
		t.Fatalf("other user create status = %d", otherMessageRecorder.Code)
	}
...
type blockingRealtimeMembershipRepository struct{ repository }

func (r blockingRealtimeMembershipRepository) IsChannelMember(ctx context.Context, userID, channelID string) (bool, error) {
	<-ctx.Done()
	return false, ctx.Err()
}

func (r blockingRealtimeMembershipRepository) ListChannelMemberIDs(ctx context.Context, channelID string) (map[string]struct{}, error) {
	<-ctx.Done()
	return nil, ctx.Err()
}

func TestRealtimeMembershipLookupsUseDeadline(t *testing.T) {
	previousTimeout := realtimeRepositoryTimeout
	realtimeRepositoryTimeout = 10 * time.Millisecond
	t.Cleanup(func() { realtimeRepositoryTimeout = previousTimeout })

	server := newServerWithRepository(blockingRealtimeMembershipRepository{repository: newMemoryRepository()})
	startedAt := time.Now()
	server.broadcast(realtimeEvent{Type: "message.created", ChannelID: "general"})
	if elapsed := time.Since(startedAt); elapsed > 250*time.Millisecond {
		t.Fatalf("broadcast membership lookup ran without a deadline: %s", elapsed)
	}

	client := &client{channelID: "*", user: User{ID: "u-timeout"}, server: server}
	startedAt = time.Now()
	client.handleCommand(websocketCommand{Type: "typing.started", ChannelID: "general"})
	if elapsed := time.Since(startedAt); elapsed > 250*time.Millisecond {
		t.Fatalf("typing membership lookup ran without a deadline: %s", elapsed)
	}
}
